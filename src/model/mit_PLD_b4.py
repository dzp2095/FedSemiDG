...
class Decoder(Module):
    def __init__(self, dims, dim, class_num=2):
        ...
        self.linear_fuse34 = ConvModule(embedding_dim * 2, embedding_dim, 1,
                                        norm_cfg=dict(type='BN', requires_grad=True))
        self.linear_fuse2  = ConvModule(embedding_dim * 2, embedding_dim, 1,
                                        norm_cfg=dict(type='BN', requires_grad=True))
        self.linear_fuse1  = ConvModule(embedding_dim * 2, embedding_dim, 1,
                                        norm_cfg=dict(type='BN', requires_grad=True))
...
class mit_PLD_b4(nn.Module):
    def __init__(self, class_num=2, fp_rate: float = 0.0, **kwargs):
